...
export const GameOverScreen: React.FC<GameOverScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const performSave = async () => {
        // If no Farcaster user, don't attempt to save anything
        if (!farcasterUser) {
  ...
  }, [farcasterUser]); // Re-run if user context changes
  ...
  const hasStamina = farcasterUser ? stamina > 0 : true;
    ...
    <div className={`absolute inset-0 flex flex-col items-center justify-start backdrop-blur-md z-50 p-4 pt-12 overflow-y-auto ${isGameClear ? 'bg-yellow-900/90' : 'bg-red-900/90'}`}>
      ...
      <div className={`w-full max-w-lg bg-white/95 rounded-3xl p-4 md:p-6 shadow-2xl border-4 ${isGameClear ? 'border-yellow-400' : 'border-red-500'} text-center animate-bounce-in my-8 relative flex-shrink-0 backdrop-blur-sm`}>
            ...
            {!claimResult ? (
              <div className="flex flex-col items-center">
                  ...
                  {walletAddress ? (
                    ...
                    <div className="text-center py-2 italic text-gray-500 text-xs">
                       Connect Farcaster wallet to claim rewards.
                    </div>
                  )}
              </div>
            ) : (
        ...
        <div className="space-y-4">
          ...
          {farcasterUser && recentHistory.length > 0 && (
          ...
          >
            {hasStamina ? (
              <>
                <RotateCcw /> TRY AGAIN 
                {farcasterUser && (
                  <span className="text-sm bg-black/20 px-2 rounded flex items-center">
                    <Zap size={14} fill="currentColor"/> {stamina}
                  </span>
                )}
              </>
            ) : (
