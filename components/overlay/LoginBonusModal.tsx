...
const THEME = {
  ...
  TOTAL_STRIP: 50,
  COLORS: {
    [ItemType.MAX_HP]: { icon: <Heart fill="currentColor" size={32} className="text-red-500" />, label: "Vitality", desc: "Start with 4 Lives", bg: "bg-red-500/20", border: "border-red-500" },
    ...
    [ItemType.SHIELD]: { icon: <Shield size={32} className="text-blue-400 fill-blue-400" />, label: "Shield", desc: "Active Shield", bg: "bg-blue-500/20", border: "border-blue-500" },
    [ItemType.NONE]: { icon: <Gift size={32} className="text-gray-400" />, label: "Nothing", desc: "Empty", bg: "bg-gray-700", border: "border-gray-500" }
  }
};
...
export const LoginBonusModal: React.FC<LoginBonusModalProps> = ({ 
  ...
  const handleClaim = useCallback(async () => {
    ...
    if (result.success) setRouletteState('SUCCESS');
    else { setClaimError(result.message); setRouletteState('RESULT'); }
  }, [resultItem, walletAddress, onClaim]);

  const config = resultItem ? THEME.COLORS[resultItem] : THEME.COLORS[ItemType.NONE];

  return (
    <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <div className="bg-gradient-to-b from-gray-900 to-gray-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl border-2 border-yellow-500/50 flex flex-col items-center">
        <div className="flex justify-between items-center w-full mb-6">
           ...
           {['IDLE', 'RESULT'].includes(rouletteState) && (
             <button onClick={onClose} className="p-2 hover:bg-gray-700 rounded-full transition-colors text-white"><X size={24} /></button>
           )}
        </div>
        ...
        {rouletteState === 'SUCCESS' && (
          <div className="flex flex-col items-center animate-in zoom-in duration-300 w-full">
            ...
            <div className={`w-20 h-20 rounded-xl ${config.bg} border-2 ${config.border} flex items-center justify-center mb-2 shadow-lg`}>{config.icon}</div>
            <div className="text-xl font-bold text-gray-200 mb-6">{config.label}</div>
            <button onClick={onClose} className="w-full py-3 bg-white text-gray-900 rounded-xl font-bold hover:bg-gray-200">CLOSE</button>
          </div>
        )}
          ...
          <>
            {resultItem ? (
              <div className="flex flex-col items-center animate-in zoom-in duration-500 mb-6">
                <div className="text-yellow-400 text-sm font-bold uppercase mb-2">Winner!</div>
                <div className={`w-24 h-24 rounded-2xl ${config.bg} ${config.border} border-2 flex items-center justify-center mb-2 shadow-xl`}>{config.icon}</div>
                <div className="text-2xl font-black text-white">{config.label}</div>
              ...
              <div className="relative w-[300px] h-[120px] bg-black/50 rounded-xl border-2 border-gray-600 overflow-hidden mb-8 shadow-inner">
                <div className="absolute top-0 bottom-0 left-1/2 w-[4px] bg-red-500 z-20 transform -translate-x-1/2"></div>
                <div ref={scrollContainerRef} className="absolute top-0 left-0 h-full flex items-center" style={{ willChange: 'transform' }}>
                  {rouletteItems.map((item, idx) => (
                    <div key={idx} className="flex-shrink-0 flex items-center justify-center" style={{ width: `${THEME.ITEM_WIDTH}px`, height: '100%' }}>
                      <div className="w-[80px] h-[80px] rounded-lg border border-gray-600 bg-gray-800 flex items-center justify-center opacity-80">{THEME.COLORS[item].icon}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {claimError && (
              <div className="w-full bg-red-900/50 border border-red-500 rounded-lg p-3 mb-4 flex gap-2 text-red-200 text-xs">
                <AlertCircle size={16} className="shrink-0" />
                <span className="break-words w-full">{claimError}</span>
              </div>
            )}

            {!resultItem ? (
              <button onClick={handleSpin} disabled={isSpinning} className={`px-8 py-3 rounded-full font-black text-xl shadow-lg transition-transform ${isSpinning ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-yellow-500 to-orange-600 text-white hover:scale-105 active:scale-95'}`}>
                {isSpinning ? 'SPINNING...' : 'SPIN ROULETTE'}
              </button>
            ) : (
              <button onClick={handleClaim} disabled={rouletteState === 'CLAIMING'} className={`w-full py-4 rounded-xl font-black text-lg shadow-lg flex items-center justify-center gap-2 transition-all ${rouletteState === 'CLAIMING' ? 'bg-blue-800 text-blue-200 cursor-wait' : 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:scale-[1.02]'}`}>
                {rouletteState === 'CLAIMING' ? <><Loader2 size={24} className="animate-spin" />WAITING...</> : <><Wallet size={24} />CLAIM</>}
              </button>
            )}
            
            {rouletteState === 'RESULT' && <div className="text-[10px] text-gray-400 mt-4 text-center">Resets daily at 00:00 UTC.</div>}
          </>
        )}
      </div>
    </div>
